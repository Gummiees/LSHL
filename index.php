...
if (isset($_GET['q'])) {
	...
	$id = $_GET['id'];
	$q = "SELECT U.user_id, U.username, U.description AS user_desc, U.image AS profile_pic, U.telephone, U.email, F.name, F.description, F.price, F.images, F.status FROM figures AS F INNER JOIN users AS U ON U.user_id = F.user_id WHERE U.user_id=$id ORDER BY F.published DESC";		
	$r = @mysqli_query ($dbc, $q);
	$num = mysqli_num_rows($r);
	if ($num > 0) {
	...
	$q = "SELECT U.user_id, U.username, F.name, F.description, F.price, F.images, F.status FROM figures AS F
	...
	if ($num > 0) {
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			if (1 == $row['status']) {
				...
				echo '<div class="card"><img class="card-img-top" src="'.$row['images'].'" alt="'.$row['name'].'"><div class="card-body"><h4 class="card-title text-center">'.$row['name'].'</h4></div><div class="card-footer"><small><a href="index.php?id='.$row['user_id'].'">'.$row['username'].'</a></small><div class="card-price"><b>'.$row['price'].'€</b></div></div></div>';
				if ($cont%4==3) {
					echo '</div>';
				}
				$cont++;
